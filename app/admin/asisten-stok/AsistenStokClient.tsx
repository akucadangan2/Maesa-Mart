...
import { Search, Download, Bot, User, Loader2 } from "lucide-react";
...
export default function AsistenStokClient({ kategoriList }: { kategoriList: Kategori[] }) {
  ...
  function exportExcel(rows: StokRow[], namaFile: string) {
    ...
    const sheet = XLSX.utils.json_to_sheet([
      { No: `Tanggal: ${TANGGAL_HARI_INI}` } as any,
      {},
      ...data,
    ] as any);
    ...
    <div className="max-w-3xl mx-auto flex flex-col h-[calc(100vh-2rem)]">
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        <button
          onClick={() =>
            jalankanAksi(
              "Daftar belanjaan: stok menipis & habis, urut A-Z",
        ...
        <button
          onClick={() =>
            jalankanAksi("Daftar belanjaan: produk paling laku (30 hari)", () =>
      ...
      {/* ===== Chat area ===== */}
      <div className="flex-1 overflow-y-auto bg-white border rounded-xl p-4 space-y-4 mb-3">
        {messages.map((m) => (
                  ...
                  <table className="w-full text-xs min-w-[600px]">
                    <thead className="bg-gray-50 text-gray-600">
                      <tr>
                        ...
                        {m.showTerjual && <th className="p-2 text-right">Terjual</th>}
                      </tr>
                    </thead>
                    <tbody>
                      {m.rows.map((r) => (
